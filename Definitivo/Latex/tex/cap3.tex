...
\begin{multicols}{2}
    \begin{enumerate}%[itemsep=0.5cm]% The item sep is my choice here.
    \item Caso $P(X=n)$
    \item Caso $P(X\leq n)$
    \item Caso $P(X<n)$
    \item Caso $P(X\geq n)$
...
Como se puede observar en el tabla \ref{tab:1} los errores que se producen entre la función implementada en Python y la calculadora de precisión, o las operaciones ''echas a mano'', es mínima. Esta observación nos induce a afirmar que esta función tiene un grado de exactitud muy alto, y por consiguiente podríamos utilizarla en cualquier problema con el mismo esquema que los estudiados en este estudio.

Además  (se analizará después de haber creado toda la gráfica y se estudiará en su conjunto),como se puede observar, tanto en el gráfico como en la tabla del tiempo (Ver \ref{time}) de ejecución, nuestra función es eficaz y eficiente para el cálculo de la probabilidad. 